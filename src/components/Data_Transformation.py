...
class DataTransformation:
    ...
    def remove_outliers(self, col, df):
        try:
            ...
            logging.info("Error occurred during outlier removal")
            raise CustomException(e, sys)
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            for i in numerical_features:
                train_data = self.remove_outliers(col=i, df=train_data)

            logging.info("Outliers capped on our train data")

            for i in numerical_features:
                test_data = self.remove_outliers(col=i, df=test_data)
            ...
            logging.info("Splitting train data into dependent and independent columns started")
            input_feature_train_data = train_data.drop(drop_column, axis=1)
            target_feature_train_data = train_data[target_column]

            logging.info("Splitting test data into dependent and independent columns started")
            ...
            save_object(file_path=self.transformation_config.preprocessor_obj_file_path, obj=preprocess_obj)
